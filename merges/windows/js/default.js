...
(function () {
    ...
    var injectBridgeShims =
        ...
        "addScript('common-cordova-bridge-server.js').then(function() {" +
            ...
            "addScript('sap-logon-cordova-bridge-server.js');" +
    ...
    function handleChangeFioriClientEvent() {
        ...
        handleEvent('resetSettings', function (event) {
            ...
            // Clear IDP domain cookies.
            if (sap.FioriClient) {
                sap.FioriClient.getIdpLogonURL(function (url) {
                    if (url) { clearWebviewCookies(url); }
                });
            }
    ...
    function navigateToUrl(url, smpRegContext, addFioriHeader) {
		...
		*/
        ...
        if (addFioriHeader) {
            var appendUserAgentToHeaders = function (userAgent) {
                if(userAgent === null || userAgent === undefined){
                    userAgent = 'SAPFioriClient/' + getVersionNumber();
                } 
                request.headers.append('User-Agent', navigator.userAgent + ' ' + userAgent);
            };
            var configHelper = cordova.require('cordova/confighelper');
            // Read the UserAgentSuffix configured in config.xml and append it to the User-Agent in the request header.
            configHelper.readConfig(
                function(config){
                    appendUserAgentToHeaders(config.getPreferenceValue("UserAgentSuffix"));
                },
                function (msg) {
                    console.log(msg);
                    appendUserAgentToHeaders();
                }
            );
        }
            ...
            getOptionalPlugins(function (plugins) {
                getWhiteListedURLs(function (urls) {
                    if (!isUrlWhiteListed(event.uri, fioriUrl, idpUrl, urls)) {
                        sap.Settings.noBridgeEnabled = true;
                        sap.FioriClient.buildToolbar();
                        ...
                        sap.Settings.noBridgeEnabled = false;
                        sap.FioriClient.buildToolbar();
    ...
    function onBackClick(event) {
        ...
        if (typeof webview !== "undefined" && webview.canGoBack) {
            ...
            webview.goBack();
            event.handled = true;
        } else if (typeof webview !== "undefined" && !webview.canGoBack) {
            // If cannot go back, then reload Home page. This can happen in deeplink
            // cases.
