sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.cancellationReason", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.cancellationReason").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			var formModel = this.getView().getModel("cancellationReasonModel");
			var oTableModel = this.getView().getModel("filterCancellationReason");
			oTableModel.setProperty("/itemCancellationReason", []);
			...
			oModel.read("/Cancellation_Reason", {
				...
				success: function (result) {
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemCancellationReason", result.results);
		...
		_onEdit: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("cancellationReasonModel");
			...
			var sPath;
			sPath = this.buildEntityPath("Cancellation_Reason", oProperties, "HCP_ID");
		...
		_onSelectConfirm: function (oEvent) {
			...
			var oSelectConfirm = this.getView().getModel("cancellationReasonModel");
		...
		_onSave: function (oEvent) {
			...
			oModel.createEntry("/Cancellation_Reason", {
		...
		_openDialogRegion: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.cancellationReasonFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);

			}

			var oModel = this.getView().getModel("cancellationReasonModel");

			var oModelCancellationReason = new sap.ui.model.json.JSONModel({
				HCP_ID: "",
				HCP_DESC: "",
				HCP_LAND1: "BR",
				HCP_ACTIVE: "1"
			...
			this.getView().setModel(oModelCancellationReason, "cancellationReasonModel");
		...
		_onDialogCancellationReasonCancelPress: function (oEvent) {
		...
		_onDeleteButton: function (oEvent) {
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterCancellationReason.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterCancellationReason.oModel.oData.itemCancellationReason[sIndex];
			var oModel = this.getOwnerComponent().getModel();
		...
		_onEditButton: function (oEvent) {
			...
			var oModelTablePrice = new sap.ui.model.json.JSONModel({
				HCP_ID: oData.HCP_ID,
				HCP_DESC: oData.HCP_DESC,
				HCP_LAND1: oData.HCP_LAND1,
				HCP_ACTIVE: oData.HCP_ACTIVE,
				HCP_EDIT_TABLE: "1"
			});

			this.getView().setModel(oModelTablePrice, "cancellationReasonModel");
