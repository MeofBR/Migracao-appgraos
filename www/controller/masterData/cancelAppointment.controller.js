sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.cancelAppointment", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.cancelAppointment").attachPatternMatched(this.handleRouteMatched, this);
		...
		handleRouteMatched: function () {
			...
			this.getView().setModel(new sap.ui.model.json.JSONModel({
				itemCancelAppointment: []
			}), "filterCancelAppointment");
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Motivos de cancelamento", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var formModel = this.getView().getModel("cancelAppointmentModel");
			var oTableModel = this.getView().getModel("filterCancelAppointment");
			oTableModel.setProperty("/itemCancelAppointment", []);
			...
			oModel.read("/Cancel_Reason", {
				...
				success: function (result) {
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemCancelAppointment", result.results);
		...
		_onEdit: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("cancelAppointmentModel");
			...
			var oProperties = {
				HCP_CNC_RSN_ID: this.arrayEdit.oData.HCP_CNC_RSN_ID,
				HCP_CNC_RSN_DESC: oData.HCP_CNC_RSN_DESC,
				HCP_ACTIVE: "1"
			};
			...
			var sPath;
			sPath = this.buildEntityPath("Cancel_Reason", oProperties, "HCP_CNC_RSN_ID");
		...
		_onSelectConfirm: function (oEvent) {
			
			oEvent.getSource().getParent().close();
			
			var oSelectConfirm = this.getView().getModel("cancelAppointmentModel");
			var oData = oSelectConfirm.getProperty("/");
			
			if(oData.HCP_EDIT_TABLE !== '1'){
				this._onSave();
			} else {
				this._onEdit();
			}
		...
		_onSave: function (oEvent) {
			...
			var oProperties = {
				HCP_CNC_RSN_ID: sTimestamp.toFixed(),
				HCP_CNC_RSN_DESC: oData.HCP_CNC_RSN_DESC,
				HCP_ACTIVE: "1"
			};
			
			oModel.createEntry("/Cancel_Reason", {
		...
		_openDialogRegion: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.cancelAppointmentFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);

			}

			var oModel = this.getView().getModel("cancelAppointmentModel");

			var oModelCancelAppointment = new sap.ui.model.json.JSONModel({
				HCP_CNC_RSN_ID: "",
				HCP_CNC_RSN_DESC: "",
				HCP_ACTIVE: "1"
			});

			this.getView().setModel(oModelCancelAppointment, "cancelAppointmentModel");
		...
		_onDialogCancelAppointmentCancelPress: function (oEvent) {
		...
		_onDeleteButton: function (oEvent) {
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterCancelAppointment.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterCancelAppointment.oModel.oData.itemCancelAppointment[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_CNC_RSN_ID: oData.HCP_CNC_RSN_ID,
								HCP_CNC_RSN_DESC: oData.HCP_CNC_RSN_DESC,
								HCP_ACTIVE: "0"
							};
		...
		_onEditButton: function (oEvent) {
			...
			this.getView().setModel(oModelTablePrice, "cancelAppointmentModel");
