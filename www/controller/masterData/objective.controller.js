sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.objective", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.objective").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Objetivos de Interação", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var oTableModel = this.getView().getModel("filterObjective");
			oTableModel.setProperty("/itemObjective", []);
			...
			oModel.read("/Interact_Objectives", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemObjective", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("objctiveFormModel");
			...
			var oProperties = {
				HCP_INT_OBJ_ID: sTimestamp.toFixed(),
				HCP_INT_OBJ_DESC: oData.HCP_INT_OBJ_DESC,
				HCP_ACTIVE: "1"
			};

		
		
			oModel.createEntry("/Interact_Objectives", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.objectiveFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "objctiveFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterObjective.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterObjective.oModel.oData.itemObjective[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_INT_OBJ_ID: oData.HCP_INT_OBJ_ID,
								HCP_ACTIVE: "0"
							};
				
						
							sPath = this.buildEntityPath("Interact_Objectives", oProperties, "HCP_INT_OBJ_ID");
