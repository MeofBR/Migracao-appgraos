sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.supplierExtract.offerMap.Edit", {
		...
		onInit: function () {
			...
			this.oRouter.getTarget("supplierExtract.offerMap.Edit").attachDisplay(this.handleRouteMatched, this);
		...
		checkPurchaseOrg: function () {
			...
			if (sEkorg) {
				if (oProfileData.ekorg.filter(ekorg => ekorg.VALOR == sEkorg).length > 0 || oProfileData.fullAccess) {
				...
				oModelOffer.read("/Offer_Map_Werks", {
					...
					success: function (results) {
						...
						for (var i = 0; i < aResults.length; i++) {
							...
							oMainDataForm[53].addContent(oCharTemplate);
		...
		_valideInputNumber: function (oProperty, oValue) {
		...
		_onCancelOfferButton: function (oEvent) {
			...
			if (!this._FragmentCancelReason) {
				...
				var oModelCancel = new JSONModel({
					HCP_STATUS: oData.HCP_STATES_OFFER,
					HCP_CANCEL_REASON: "",
		...
		_onCancelPress: function (oEvent) {
			...
			var oCancelModel = this.getView().getModel("editOfferMapFormModel");
		...
		_onConfirPress: function (oEvent) {
			...
			if (oCancelData.HCP_STATUS == "2" || oCancelData.HCP_STATUS == "5") { //Comprado Parcialmente ou Erro
					...
					oModel.submitChanges({
						...
						success: function () {
							if ((bIsMobile && navigator.connection.type !== "none") || !bIsMobile) {
								this.flushStore("Offer_Map,Commodities_Fixed_Order,Commodities_Order").then(function () {
									this.refreshStore("Offer_Map", "Commodities_Fixed_Order", "Commodities_Order").then(function () {
										MessageBox.success(
											oMessageSucess, {
		...
		_onInputArmazemFormSelect: function (oEvent) {
			...
			if (oInput.getSelectedKey() === "1") { //Próprio
		...
		_onInputIncotermFormSelect: function (oEvent) {
			...
			this.clearContainers("newCenterSimpleForm").then(function () {
				this.insertTemplateCenter().then(function () {
					...
					if (oInput.getSelectedKey() === "1") { //CIF
		...
		buildCenterTemplate: function () {
			...
			if (oModel.oData.HCP_INCOTERM == "1") { //CIF
				var oCif = true;
				var oFob = false;
			} else { //FOB
				oCif = false;
		...
		buildEntityCenterPath: function (sEntityName, oEntity) {
			...
			if (oEntity["@com.sap.vocabularies.Offline.v1.isLocal"] === true) {
				...
				return "/" + sEntityName + "(HCP_WERKS='" + oEntity.HCP_WERKS + "',HCP_UNIQUE_KEY='" + oEntity.HCP_UNIQUE_KEY + "')";
			}
		},
		...
		refreshStore: function (entity) {
			return new Promise(function (resolve, reject) {
				if (typeof sap.hybrid !== 'undefined') {
					this.setBusyDialog(this.resourceBundle.getText("textOfferMap"), this.resourceBundle.getText("messageUpdateDatabase"));
		...
		commoditiesData: function (oOffer) {
			...
			if (oData.HCP_CREATE_OFFER == "1") { //Criar Compra
				...
				if (oData.HCP_PARTNER_TYPE == "1") { //Fornecedor
					...
					if (oData.ItemLocal.length > 0 && oData.HCP_LOCAL) {

						if (oData.HCP_LOCAL.length < 10) {
							var oNum = 10 - oData.HCP_LOCAL.length;

							while (oNum != 0) {
								oData.HCP_LOCAL = "0" + oData.HCP_LOCAL;
								oNum = oNum - 1;
							}
						}

						oLocalArray = oData.ItemLocal.filter(result => result.LIFNR === oData.HCP_LOCAL);
					}

				} else { //Prospect

					oLocalArray[0].LIFNR = oData.LIFNR_PROSPECT;

				}

				if (oLocalArray.length > 1) {
					oEnabledLocal = true;
				} else {
					var oLocal = oLocalArray[0].LIFNR;
				}
			...
			this._checkFreightCalculation(oData.inputCenter).then(function () {
				...
				for (var i = 0; i < oData.inputCenter.length; i++) {
					...
					var aDataCenter = {
						...
						HCP_UNIQUE_KEY: this.uniqueKey,
						HCP_PRICE_OFFER: oData.HCP_INCOTERM == '1' ? parseFloat(oData.inputCenter[i].HCP_PRICE_OFFER).toFixed(2) : "0.00",
		...
		_onPurchaseOrgChange: function (oEvent) {
			...
			if (oProfileData.ekorg.filter(ekorg => ekorg.VALOR == oInputValue).length > 0 || oProfileData.editAll) {
				oOfferMapModel.setProperty("/enableEditPurchaseOrgValid", true);
			} else {
				oOfferMapModel.setProperty("/enableEditPurchaseOrgValid", false);
			}
