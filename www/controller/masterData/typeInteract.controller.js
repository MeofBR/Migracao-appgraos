sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.typeInteract", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.typeInteract").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Tipos de Interação", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var oTableModel = this.getView().getModel("filterTypeInteract");
			oTableModel.setProperty("/itemTypeInteract", []);
			...
			oModel.read("/Interact_Types", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemTypeInteract", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("typeInteractFormModel");
			...
			var oProperties = {
				HCP_INT_TYPE_ID: sTimestamp.toFixed(),
				HCP_INT_TYPE_DESC: oData.HCP_INT_TYPE_DESC,
				HCP_ACTIVE: "1"
			};

		
		
			oModel.createEntry("/Interact_Types", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.typeInteractFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "typeInteractFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterTypeInteract.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterTypeInteract.oModel.oData.itemTypeInteract[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_INT_TYPE_ID: oData.HCP_INT_TYPE_ID,
								HCP_ACTIVE: "0"
							};
				
						
							sPath = this.buildEntityPath("Interact_Types", oProperties, "HCP_INT_TYPE_ID");
