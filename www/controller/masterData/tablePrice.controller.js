sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.tablePrice", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.tablePrice").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function () {
			...
			var oTableModel = this.getView().getModel("filterTablePrice");
			oTableModel.setProperty("/itemPrice", []);
			...
			oModel.read("/Table_Price_Parameters", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemPrice", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oProperties = {
				HCP_TABLE_PRICE_ID: sTimestamp.toFixed(),
			...
			oModel.createEntry("/Table_Price_Parameters", {
		...
		_onPriceEditButton: function (oEvent) {
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterTablePrice.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterTablePrice.oModel.oData.itemPrice[sIndex];

			
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.tablePriceFragment",
					this);
			...
			var oModelTablePrice = new sap.ui.model.json.JSONModel({
				HCP_TABLE_PRICE_ID: oData.HCP_TABLE_PRICE_ID,
				HCP_POSITION: oData.HCP_POSITION,
				HCP_MATERIAL: oData.HCP_MATERIAL,
				HCP_MATERIAL_DESCRIPTION: oData.HCP_MATERIAL_DESCRIPTION,
				HCP_TYPE_MATERIAL: oData.HCP_TYPE_MATERIAL,
				HCP_TYPE_MATERIAL_DESCRIPTION: oData.HCP_TYPE_MATERIAL_DESCRIPTION,
			...
			this.getView().setModel(oModelTablePrice, "tablePriceFormModel");
		...
		_openDialogTablePrice: function () {
			...
			var oModelTablePrice = new sap.ui.model.json.JSONModel({
				...
				HCP_TYPE_MATERIAL: "",
				HCP_TYPE_MATERIAL_DESCRIPTION: "",
				HCP_GROUP_DESCRIPTION: "",
				HCP_GROUP_COLOR: "",
				HCP_CREATED_BY: this.userName,
				HCP_CREATED_AT: this._formatDate(new Date())
			});
				...
				"Deseja Deletar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							sPath = this.buildEntityPath("Table_Price_Parameters", oModelTablePrice.oData, "HCP_TABLE_PRICE_ID");
