sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.commit", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.commit").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Compromissos", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var oTableModel = this.getView().getModel("filterCommit");
			oTableModel.setProperty("/itemCommit", []);
			...
			oModel.read("/Commitments", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemCommit", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("commitFormModel");
			...
			var oProperties = {
				HCP_COMMITMENT_ID: sTimestamp.toFixed(),
				HCP_COMMITMENT_DESC: oData.HCP_COMMITMENT_DESC,
				HCP_ACTIVE: "1"
			};

		
		
			oModel.createEntry("/Commitments", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.commitFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "commitFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterCommit.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterCommit.oModel.oData.itemCommit[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_COMMITMENT_ID: oData.HCP_COMMITMENT_ID,
								HCP_ACTIVE: "0"
							};
				
						
							sPath = this.buildEntityPath("Commitments", oProperties, "HCP_COMMITMENT_ID");
