sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.visitForm.EditPeriodicVisitForm", {

		onInit: function () {

			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getTarget("visitForm.EditPeriodicVisitForm").attachDisplay(this.handleRouteMatched, this);
		...
		handleRouteMatched: function (oEvent) {
			...
			var oEditModel = this.getView().getModel("editPeriodicVisitModel");
			...
			this.searchValuesStorageProduct(oModel, false);
		...
		getUserProfileMasterData: async function (sUserName) {
			var oModel = this.getView().getModel();
			var oVisitFormModel = this.getView().getModel("editPeriodicVisitModel");
		...
		getCropYear: async function () {
			...
			let oModelCropYear = this.getView().getModel("editPeriodicVisitModel")
            let oFilters = [];
			...
			oModelVisit.read("/Visit_Culture_Type", {
				...
				success: function (results) {
					...
					for (var i = 0; i < aResults.length; i++) {
						...
						oMainDataForm[10].addContent(new sap.m.Label({
							text: ""
						}));

						oMainDataForm[10].addContent(oCharTemplate);
						...
						oEditModel.setProperty("/cultureType/" + i + "/HCP_SAFRA_PERCENTAGE", aResults[i].HCP_SAFRA_PERCENTAGE);
						oEditModel.setProperty("/cultureType/" + i + "/HCP_AVAILABLE_VOLUME", aResults[i].HCP_AVAILABLE_VOLUME);
						oEditModel.setProperty("/cultureType/" + i + "/HCP_FIXED_PRICE", aResults[i].HCP_FIXED_PRICE);
						oEditModel.setProperty("/cultureType/" + i + "/HCP_OPEN_PRICE", aResults[i].HCP_OPEN_PRICE);
		...
		searchValuesStorageProduct: function (oModel, oRemove) {
			return new Promise(function (resolve, reject) {
				var oModelVisit = this.getView().getModel();
				var oEditModel = this.getView().getModel("editPeriodicVisitModel");
				var oData = oEditModel.oData;
				...
				if (!oRemove) {
					oEditModel.setProperty("/storedProductInputs", []);
				} else {
					...
					success: function (results) {
						...
						for (var i = 0; i < aResults.length; i++) {
							...
							if (!oRemove) {
								...
								oEditModel.setProperty("/HCP_DESC_STORAGE_TYPE", aResults[i].HCP_DESC_STORAGE_TYPE);
						...
						if (!oRemove) {
							oEditModel.setProperty("/storedProductInputs", negotiationInputs);
						...
						for (var i = 0; i < aResults.length; i++) {
							...
							if (!oRemove) {
								...
								if (aResults[i].HCP_CRITERION === '6') {
									...
									oEditModel.setProperty("/HCP_OTHERS_CRITERION", aResults[i].HCP_OTHERS);
								}
		...
		onCancelPress: function () {
			var oVisitModel = this.getView().getModel("editPeriodicVisitModel");
		...
		_calculateCommercializationCulture: async function (oProperties) {
			...
			const propertiesCulture = {
				...
				HCP_PRODUCTIVITY_TOTAL: oProperties.HCP_PRODUCTIVITY_TOTAL == undefined ? '0' : oProperties.HCP_PRODUCTIVITY_TOTAL.toString(),
				HCP_COMMERCIALIZED_CROP: oProperties.HCP_SAFRA_PERCENTAGE == undefined ? '0' : Number(oProperties.HCP_SAFRA_PERCENTAGE) > 100 ?
					'100' : oProperties.HCP_SAFRA_PERCENTAGE.toString(),
				HCP_NEW_CROP: oProperties.HCP_AVAILABLE_VOLUME == undefined ? '0' : Number(oProperties.HCP_AVAILABLE_VOLUME) < 0 ? '0' : oProperties
					.HCP_AVAILABLE_VOLUME.toString(),
				HCP_DESCRIPTION: ''
			}
			...
			this.searchValuesTools(oModel, true).then(function () {
				this.searchValuesCertifications(oModel, true).then(function () {
					this.searchValuesCriterions(oModel, true).then(function () {
						this.searchValuesStorageProduct(oModel, true).then(function () {
		...
		redirectYearly: function (oEvent) {
			...
			var aFilters = [];
			var aSortes = [];
			var oCreateModelPeriodic = this.getView().getModel("editPeriodicVisitModel");
		...
		onSubmitChanges: async function (oEvent) {
			...
			if (this.period === oData.HCP_PERIOD) {
				...
				sPath = this.buildEntityPath("Visit_Form_Periodic", oData);
			...
			for (var i = 0; i < oData.certificationsInputs.length; i++) {
				let numberRandom = this.generateRandomNumber()
				var sCertificationKey = new Date().getTime() + sCounter + numberRandom;
				sCounter = sCounter + 1;

				var aDataCertificationsInput = {
					HCP_VISIT_ID: sCertificationKey,
					HCP_PROVIDER_ID: oData.HCP_PROVIDER_ID,
					HCP_CONTACT_TYPE: oData.HCP_CONTACT_TYPE,
					HCP_CONTACT_INICIATIVE: oData.HCP_CONTACT_INICIATIVE,
					HCP_VISIT_FORM: oData.HCP_VISIT_FORM,
			...
			for (var i = 0; i < oData.criterionsInputs.length; i++) {
				let numberRandom = this.generateRandomNumber()
				var sCriterionKey = new Date().getTime() + sCounter + numberRandom;
				sCounter = sCounter + 1;

				var aDataCriterionsInputs = {
					HCP_VISIT_ID: sCriterionKey,
					HCP_PROVIDER_ID: oData.HCP_PROVIDER_ID,
					HCP_CONTACT_TYPE: oData.HCP_CONTACT_TYPE,
					HCP_CONTACT_INICIATIVE: oData.HCP_CONTACT_INICIATIVE,
					HCP_VISIT_FORM: oData.HCP_VISIT_FORM,
			...
			for (var i = 0; i < oData.storedProductInputs.length; i++) {
				let numberRandom = this.generateRandomNumber()
				var sStoreKey = new Date().getTime() + sCounter + numberRandom;
				sCounter = sCounter + 1;

				var aDataNegotiationInput = {
					HCP_VISIT_ID: sStoreKey,
					HCP_PROVIDER_ID: oData.HCP_PROVIDER_ID,
					HCP_CONTACT_TYPE: oData.HCP_CONTACT_TYPE,
					HCP_CONTACT_INICIATIVE: oData.HCP_CONTACT_INICIATIVE,
					HCP_VISIT_TYPE: "Periodic",
			...
			for (var i = 0; i < oData.toolsInputs.length; i++) {
				let numberRandom = this.generateRandomNumber()
				var sToolKey = new Date().getTime() + sCounter + numberRandom;
				sCounter = sCounter + 1;

				var aDataToolsInput = {
					HCP_VISIT_ID: sToolKey,
					HCP_PROVIDER_ID: oData.HCP_PROVIDER_ID,
					HCP_CONTACT_TYPE: oData.HCP_CONTACT_TYPE,
					HCP_CONTACT_INICIATIVE: oData.HCP_CONTACT_INICIATIVE,
					HCP_VISIT_FORM: oData.HCP_VISIT_FORM,
			...
			for (var i = 0; i < oData.cultureType.length; i++) {
				let numberRandom = this.generateRandomNumber()
				var sCultureKey = new Date().getTime() + sCounter + numberRandom;
				sCounter = sCounter + 1;
				...
				if (this.period === oData.HCP_PERIOD) {
					...
					if (oData.cultureType[i].status === "New") {
						aDataCultureType["HCP_VISIT_ID"] = sCultureKey;
						...
						this._calculateCommercializationCulture({...aDataCultureType,
							oData
						})
						...
						this.flushStore(
							...
							this.refreshStore("Visit_Form_Periodic", "Visit_Form_Certifications", "Visit_Storage_Type",
								...
								if (this.validateSubmiteChange(data))
									this.updateSimplifyContact(this.getView().getModel("editPeriodicVisitModel").oData.HCP_PROVIDER_ID, this.getView().getModel(
										"editPeriodicVisitModel").oData.HCP_NAME_REGISTERED);

								if (await this.prepareEqualize(this.getView().getModel("editPeriodicVisitModel").oData, 'Periodic')) {
									MessageBox.success(
		...
		_calculateTotalBalance: function (oProperty) {
			...
			var oModelYearly = this.getView().getModel("editPeriodicVisitModel");
			...
			let qtdeCommercialized = oData.HCP_PRODUCTIVITY_TOTAL * (oData.HCP_SAFRA_PERCENTAGE / 100);
			sTotal = Number(oData.HCP_PRODUCTIVITY_TOTAL) - qtdeCommercialized;
		...
		buildCultureTypeTemplate: function () {
			...
			var oeditPeriodicVisitModel = this.getView().getModel("editPeriodicVisitModel");
			var oData = oeditPeriodicVisitModel.oData;
			var sChars = oeditPeriodicVisitModel.getProperty("/cultureType");
			...
			if (!sChars) {
				oeditPeriodicVisitModel.setProperty("/cultureType", []);
			}

			var sCharLength = oeditPeriodicVisitModel.getProperty("/cultureType").length;
			oeditPeriodicVisitModel.setProperty("/cultureType/" + sCharLength, {});
			oeditPeriodicVisitModel.setProperty("/cultureType/" + sCharLength + "/status", "New");
			...
			oeditPeriodicVisitModel.setProperty("/hasPermissionToEditOldSafra", undefined)

			let oTemplate = new sap.m.VBox({
				fitContainer: true,
					...
					new sap.ui.layout.form.SimpleForm({
						...
						content: [
							...
							new CustomComboBox({
								...
								selectedKey: "{editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_CULTURE_TYPE}",
							...
							new CustomComboBox({
								showSecondaryValues: true,
								selectedKey: "{editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_SAFRA_YEAR}",
							...
							new sap.m.Input({
								value: "{ path: 'editPeriodicVisitModel>/cultureType/" + sCharLength +
							...
							new sap.m.Input({
								value: "{ path: 'editPeriodicVisitModel>/cultureType/" + sCharLength +
									"/HCP_PRODUCTIVITY_TOTAL' , type: 'sap.ui.model.type.Float', formatOptions: { groupingEnabled: true, groupingSeparator: '.', decimalSeparator: ',', decimalPlaces: 2, maxFractionDigits: 2 }}",
								type: "Tel",
								design: "Standard",
							...
							new com.sap.build.standard.brfAppDeGraosModoEditar.controls.CustomSlider({
								min: 0,
								max: 100,
								value: "{editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_SAFRA_PERCENTAGE}",
								Step: 1,
								progress: true,
								enabled: "{editPeriodicVisitModel>/isEnable}",
								editable: "{editPeriodicVisitModel>/isEnable}",
							...
							new sap.m.NumericContent({
								...
								value: "{= ${editPeriodicVisitModel>/cultureType/" + sCharLength +
									"/HCP_SAFRA_PERCENTAGE} > 100 ? '100' : (${editPeriodicVisitModel>/cultureType/" + sCharLength +
									"/HCP_SAFRA_PERCENTAGE} < 0 ? '0' : ${editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_SAFRA_PERCENTAGE})}",
								scale: "%",
								withMargin: true,
								nullifyValue: true,
								layoutData: [
							...
							new sap.m.Input({
								value: "{= ${editPeriodicVisitModel>/cultureType/" + sCharLength +
									"/HCP_AVAILABLE_VOLUME} < 0 ? '0' : ${path: 'editPeriodicVisitModel>/cultureType/" + sCharLength +
							...
							new com.sap.build.standard.brfAppDeGraosModoEditar.controls.CustomSlider({
								min: 0,
								max: 100,
								value: "{editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_FIXED_PRICE}",
								Step: 1,
								progress: true,
								width: "100%",
							...
							new com.sap.build.standard.brfAppDeGraosModoEditar.controls.CustomSlider({
								min: 0,
								max: 100,
								value: "{editPeriodicVisitModel>/cultureType/" + sCharLength + "/HCP_OPEN_PRICE}",
								Step: 1,
								progress: true,
								width: "100%",
								name: "HCP_OPEN_PRICE",
			...
			oeditPeriodicVisitModel.oData.cultureType[sCharLength].HCP_SAFRA_PERCENTAGE = 0;
		...
		getCultureTypes: function (sUniqueKey) {
			var oModel = this.getView().getModel("editPeriodicVisitModel");
		...
		_calculateTotalPrice: function (oProperty) {
			...
			var oModelPeriodic = this.getView().getModel("editPeriodicVisitModel");
			var oPrice;
			var oSource = oProperty.getSource();
			var sName = oSource.getName();
			var sPath = oSource.getParent().getParent().getParent().getParent().getParent().getCustomData()[0].getValue();
		...
		_validateFormSpecialCharacters: function (sErro) {
			...
			setTimeout(function () {
				var oFilterModel = this.getView().getModel("editPeriodicVisitModel");
		...
		removeCultureType: function (oEvent) {
			var oPropertyModel = this.getView().getModel("editPeriodicVisitModel");
