sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.sourceInformation", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.sourceInformation").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Certificados", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var oTableModel = this.getView().getModel("filterSourceInformation");
			oTableModel.setProperty("/itemSourceInformation", []);
			...
			oModel.read("/Source_Information", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemSourceInformation", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("sourceInformationFormModel");
			...
			var oProperties = {
				HCP_SOURCE_ID: sTimestamp.toFixed(),
			...
			oModel.createEntry("/Source_Information", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.sourceInformationFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "sourceInformationFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterSourceInformation.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterSourceInformation.oModel.oData.itemSourceInformation[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_SOURCE_ID: oData.HCP_SOURCE_ID,
								HCP_ACTIVE: "0"
							};
				
							sPath = this.buildEntityPath("Source_Information", oProperties, "HCP_SOURCE_ID");
