sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.depositCondition", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.depositCondition").attachPatternMatched(this.handleRouteMatched, this);
		...
		handleRouteMatched: function () {
			...
			this.getView().setModel(new sap.ui.model.json.JSONModel({
				itemDepositCondition: []
			}), "filterDepositCondition");
		...
		getParameters: function (status) {
			...
			var oTableModel = this.getView().getModel("filterDepositCondition");
			oTableModel.setProperty("/itemDepositCondition", []);
			...
			oModel.read("/Deposit_Condition", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemDepositCondition", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("depositConditionFormModel");
			...
			oModel.createEntry("/Deposit_Condition", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.depositConditionFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "depositConditionFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterDepositCondition.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterDepositCondition.oModel.oData.itemDepositCondition[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							sPath = this.buildEntityPath("Deposit_Condition", oProperties, "HCP_ID");
