sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.conditions", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.conditions").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			var oTableModel = this.getView().getModel("filterConditions");
			oTableModel.setProperty("/itemConditions", []);
			...
			oModel.read("/Crop_Conditions", {
				...
				success: function (result) {
					...
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemConditions", result.results);
		...
		_onSave: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("conditionsFormModel");
			...
			var oProperties = {
				HCP_COND_ID: sTimestamp.toFixed(),
				HCP_LAND1: "BR",
			...
			oModel.createEntry("/Crop_Conditions", {
		...
		_openDialogCertification: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.conditionsFragment",
					this);
			...
			this.getView().setModel(oModelCertification, "conditionsFormModel");
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterConditions.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterConditions.oModel.oData.itemConditions[sIndex];
			var oModel = this.getOwnerComponent().getModel();
				...
				"Deseja Inativar o parâmetro?", {
					...
					onClose: function (oAction) {
						...
						if (oAction === "YES") {
							...
							var oProperties = {
								HCP_COND_ID: oData.HCP_COND_ID,
								HCP_ACTIVE: "0"
							};
				
						
							sPath = this.buildEntityPath("Crop_Conditions", oProperties, "HCP_COND_ID");
