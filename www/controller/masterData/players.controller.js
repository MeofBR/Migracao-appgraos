sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.players", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.players").attachPatternMatched(this.handleRouteMatched, this);
		...
		getParameters: function (status) {
			...
			this.setBusyDialog("Players", "Carregando Parâmetros...");
			var oModel = this.getView().getModel();
			var formModel = this.getView().getModel("playersModel");
			var oTableModel = this.getView().getModel("filterPlayers");
			oTableModel.setProperty("/itemPlayers", []);
			...
			oModel.read("/Players", {
				...
				success: function (result) {
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemPlayers", result.results);
		...
		_onEdit: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("playersModel");
			...
			var oProperties = {
				...
				HCP_CANAL: oData.HCP_CANAL,
				HCP_ACTIVE: "1",
				HCP_CONSUMO: oData.HCP_CONSUMO,
			...
			if (aDeferredGroups.indexOf("changes") < 0) {
	     	...
	     	var sPath;
			sPath = this.buildEntityPath("Players", oProperties, "HCP_ID");
		...
		_openDialogRegion: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.playersFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);

			}
			
			var oModel = this.getView().getModel("playersModel");
			
			var oModelPlayers = new sap.ui.model.json.JSONModel({
				HCP_ID: "",
				HCP_NAME: "",
				HCP_CANAL: "",
				HCP_ACTIVE: "1",
			...
			this.getView().setModel(oModelPlayers, "playersModel");
		...
		_onDialogPlayersCancelPress: function (oEvent) {
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterPlayers.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterPlayers.oModel.oData.itemPlayers[sIndex];
			var oModel = this.getOwnerComponent().getModel();
		...
		_onEditButton: function (oEvent) {
			...
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.playersEditFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);
			...
			this.getView().setModel(oModelTablePrice, "playersModel");
