sap.ui.define(["com/sap/build/standard/brfAppDeGraosModoEditar/controller/MainController",
	...
	return MainController.extend("com.sap.build.standard.brfAppDeGraosModoEditar.controller.masterData.masterDataPermission", {

		onInit: function () {
			this.oRouter = sap.ui.core.UIComponent.getRouterFor(this);
			this.oRouter.getRoute("masterData.masterDataPermission").attachPatternMatched(this.handleRouteMatched, this);
		...
		handleRouteMatched: function () {
			...
			this.getView().setModel(new sap.ui.model.json.JSONModel({
				itemMasterDataPermission: []
			}), "filterMasterDataPermission");


			this.getView().setModel(new sap.ui.model.json.JSONModel({
			
			}), "masterDataPermissionModel");
		...
		getParameters: function (status) {
			...
			var formModel = this.getView().getModel("masterDataPermissionModel");
			var oTableModel = this.getView().getModel("filterMasterDataPermission");
			oTableModel.setProperty("/itemMasterDataPermission", []);
			...
			oModel.read("/Master_Data_Permissions", {
				...
				success: function (result) {
					if (result.results.length > 0) {
						oTableModel.setProperty("/itemMasterDataPermission", result.results);
		...
		_onEdit: function (oEvent) {
			...
			var oCreateModel = this.getView().getModel("masterDataPermissionModel");
			...
			if (aDeferredGroups.indexOf("changes") < 0) {
	     	...
	     	var sPath;
			sPath = this.buildEntityPath("Master_Data_Permissions", oProperties, "HCP_ID");
						...
						MessageBox.success(
							"Parâmetros editados com sucesso.", {
								...
								onClose: function (sAction) {
									this.closeBusyDialog();
									this.getParameters();
									this._onDialogMasterDataPermissionCancelPress()
								}.bind(this)
							}
						);
					}.bind(this),
		...
		_onSave: function (oEvent) {
			...
			oModel.createEntry("/Master_Data_Permissions", {
		...
		_openDialogRegion: function () {
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.masterDataPermissionFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);

			}
			
			var oModel = this.getView().getModel("masterDataPermissionModel");
			...
			this.getView().setModel(oModelMasterDataPermission, "masterDataPermissionModel");
		...
		_onDialogMasterDataPermissionCancelPress: function (oEvent) {
			oEvent && oEvent.getSource().getParent().close();
			this.getView().getModel("masterDataPermissionModel").oData = {}
		...
		_onDeleteButton: function(oEvent){
			...
			var sPlit = oItem.oParent.oParent.oBindingContexts.filterMasterDataPermission.sPath.split("/");
			var sIndex = sPlit[2];
			var oData = oItem.oParent.oParent.oBindingContexts.filterMasterDataPermission.oModel.oData.itemMasterDataPermission[sIndex];
			var oModel = this.getOwnerComponent().getModel();
		...
		_onEditButton: function (oEvent) {
			...
			if (!this._FragmentPrice) {
				this._FragmentPrice = sap.ui.xmlfragment("tablePriceId" + this.getView().getId(),
					"com.sap.build.standard.brfAppDeGraosModoEditar.view.masterData.fragments.masterDataPermissionEditFragment",
					this);
				this.getView().addDependent(this._FragmentPrice);
			...
			this.getView().setModel(oModelTablePrice, "masterDataPermissionModel");
